// .wrangler/tmp/bundle-6vQeHF/checked-fetch.js
...
(function(ResponseType2) {
  ...
  })(Code = ResponseType2.Code || (ResponseType2.Code = {}));
  let FailCode;
  (function(FailCode2) {
    FailCode2[FailCode2["notFound"] = 0] = "notFound";
    FailCode2[FailCode2["falseMethod"] = 1] = "falseMethod";
    FailCode2[FailCode2["unauthorization"] = 2] = "unauthorization";
    FailCode2[FailCode2["format"] = 3] = "format";
  })(FailCode = ResponseType2.FailCode || (ResponseType2.FailCode = {}));
...
var Buffer = moduleExports ? _root_default.Buffer : void 0;
...
var isString = function(value) {
  return typeof value == "string" || !isArray_default(value) && isObjectLike_default(value) && _baseGetTag_default(value) == stringTag2;
};
...
var api = async (data) => {
  ...
  for (const key in data)
    body2.append(key, data[key]);
  const headers = new Headers();
  headers.append("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
  forEach_default(store.header, (v, k) => headers.set(k, v));
  let bodySearch = "?";
  for (const key in data) {
...
var count = async () => (await api({
  action: "count"
})).count;
var remove = async (tag) => await api({
  action: "delete",
  tag
});
var searchByUid = async (uid) => await get(`${uid}.value`);
var searchByEmail = async (email) => await get(`${email}.value`);
var getTimeByUid = async (uid) => await get(`${uid}.time`);
var getTimeByEmail = async (email) => await get(`${email}.time`);
var deleteStore = async (ids, key) => await remove(`${ids}.store.${key}`);
var getStore = async (ids, key) => await get(`${ids}.store.${key}`);
var setStore = async (ids, key, value) => {
  if (isString_default(ids))
    var u = await searchByEmail(ids);
  else
    var u = await searchByUid(ids);
  await api({
    action: "update",
...
(function(jwt) {
  ...
  jwt.secret = "vhbuioy78a32et6r7drtxfcyutfdresxyrtuyfdresxdfcgtyfui7uihfip239u0hjfaf2hf89h29fniune2iuf";
  jwt.value = "";
})(jwt3 || (jwt3 = {}));
jwt3.value = await sign2(jwt3.payload, jwt3.secret, jwt3.payload.alg);
var app = new Hono2();
...
app.post("/user", async (c) => {
  ...
  const body2 = getUser.safeParse(await c.req.json());
  const authorization = z.string().safeParse(header.authorization);
  if (authorization.success && await verify2(authorization.data, jwt3.secret, "HS512")) {
    if (body2.success) {
      try {
        switch (body2.data.type) {
          case "uid": {
            return c.json({
              code: ResponseType.Code.success,
              data: await searchByUid(body2.data.uid)
            }, 200);
          }
          case "email": {
            return c.json({
              code: ResponseType.Code.success,
              data: await searchByEmail(body2.data.email)
            }, 200);
          }
          case "pid": {
            return c.json({
              code: ResponseType.Code.success,
              data: await get(body2.data.pid)
            }, 200);
          }
        }
      } catch (error) {
  ...
  if (!webSaveDeepRule.safeParse(body2).success)
    return c.json({
      code: ResponseType.Code.fail,
      data: {
        code: ResponseType.FailCode.format,
  ...
  try {
    ...
    await api({
      data: {
        tag: `${body2.uid}.value`,
        value: webSaveRule.parse(body2),
        action: "update"
      }
    });
    await api({
      data: {
        tag: `${body2.email}.value`,
        value: webSaveRule.parse(body2),
        action: "update"
      }
    });
...
app.post("/time", async (c) => {
  const header = store.header = c.req.header();
  const body2 = getTime.safeParse(await c.req.json());
  const authorization = z.string().safeParse(header.authorization);
  if (authorization.success && await verify2(authorization.data, jwt3.secret, "HS512")) {
    if (body2.success) {
      try {
        switch (body2.data.type) {
          case "uid": {
            return c.json({
              code: ResponseType.Code.success,
              data: await getTimeByUid(body2.data.uid)
            }, 200);
          }
          case "email": {
            return c.json({
              code: ResponseType.Code.success,
              data: await getTimeByEmail(body2.data.email)
            }, 200);
          }
        }
      } catch (error) {
...
app.all("/file/*", async (c) => {
  ...
  const headers = new Headers();
  forEach_default(header, (v, k) => headers.set(k, v));
  try {
...
app.put("/user/:uid/store/*", async (c) => {
  ...
  try {
    await setStore(c.req.param().uid, key, await c.req.text());
    return c.json({
      code: ResponseType.Code.success,
  ...
  const key = c.req.path.match(/(?<=\/user\/\w+\/store\/).+/g)[0].replaceAll("/", ".");
  try {
    await deleteStore(c.req.param().uid, key);
    return c.json({
      code: ResponseType.Code.success,
      data: await c.req.json()
    });
  ...
  try {
    return c.json({
      code: ResponseType.Code.success,
      data: await getStore(c.req.param().uid, key)
    });
  } catch (error) {
    return c.json({
      code: ResponseType.Code.success,
...
// .wrangler/tmp/bundle-6vQeHF/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6vQeHF/middleware-loader.entry.ts
